...
javaプログラムの全体像
javaのソースコードには、波括弧{}で囲まれた部分が多く登場する。
この波括弧で囲まれた部分をブロックと呼ぶ。外側のブロックは、クラスブロック（class block）、内部のブロックはメソッドブロック(method block)と呼ばれ、
javaのソースコードは必ずこれらのブロックによる二重構造を持っている。
私たちがコンピュータに対して指示・命令を記述していくのはメソッドブロックの中です。
